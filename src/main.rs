...
use infinite_game_of_life::universe::Universe;
use three_d::*;

fn main() {
	let window = Window::new(WindowSettings {
		title: env!("CARGO_PKG_NAME").to_case(Case::Title),
	...
	let universe = Universe::new(window.size().0 * 8, window.size().1 * 8);
	println!("{universe:?}");
	...
	let mut line = Line::new_with_material(
		&context,
		vec2(0.0, 0.0),
		vec2(window.viewport().width as f32, window.viewport().height as f32),
		5.0,
		ColorMaterial {
			color: Color::GREEN,
			..Default::default()
